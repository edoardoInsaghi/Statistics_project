...
# We choose to exlude the variable with the highest VIF
...
#let's try another approach: exclude first the most correlated variables (really high correlation) from the dataset (by looking at the correlation matrix and the mutual information matrix) and then use the remaining variables to find the best model with stepAIC
complete_model_all = lm(SalePrice ~ ., df) #model with all covariates
...
#COMPUTATIONALLY TOO HEAVY BUT WE TRIED THIS AS WELL
#let's try another method as well
#library(leaps)

# Generate all possible model combinations

#all_models <- regsubsets(SalePrice ~ ., df, nvmax = ncol(df) - 1, method = "exhaustive", really.big = TRUE)

# Find the model with the minimum AIC
#best_model <- which.min(all_models$aic)

# Extract the variables in the best model
#best_variables <- names(coef(all_models, id = best_model))

# Fit the best model
#final_model <- lm(SalePrice ~ ., df[, c("SalePrice", best_variables)])
...
selected_test_data <- df_test[, c("LotArea","LotShape", "LandContour", "LandSlope", "Condition1", "BldgType", "OverallQual", "YearBuilt", "YearRemodAdd", "RoofStyle", "BsmtExposure", "BsmtFinSF1", "BsmtFinSF2", "TotalBsmtSF", "HeatingQC", "LowQualFinSF", "GrLivArea", "BsmtFullBath", "BedroomAbvGr", "KitchenAbvGr", "KitchenQual", "Functional", "Fireplaces", "GarageFinish", "GarageArea", "WoodDeckSF", "ScreenPorch", "PoolArea", "MoSold", "SaleCondition")]

# Make predictions using the trained final model we choosed
predictions <- predict(final_model, newdata = selected_test_data)
print(predictions)

# I can't find the SalePrice values in the test data to evaluate the predictions :(
```
