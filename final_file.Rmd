...
```{r pre processing} 
df = read.csv("/Users/edoardozappia/Desktop/statistical project/train.csv")
...
df_test = read.csv("/Users/edoardozappia/Desktop/statistical project/test.csv")
...
for (fold in 1:numfolds){
  ...
  rf_model = randomForest(SalePrice ~ ., data = train, ntree = 1000, importance = TRUE)
...
res = data.frame("rmse" = c(mse_linear, mse_gam, mse_rf),
                 "model" = c(rep("Linear", numfolds), rep("Gam", numfolds), rep("Rf", numfolds)))

res %>% ggplot(aes(y=rmse, x=model, fill=model)) + geom_boxplot()
```

```{r possible indexes to add}
#rfErrors <- prediction - real_values
#rfMSE <- mean(rfErrors^2)
#rfRMSE <- sqrt(rfMSE)
#rfMAPE <- mean(abs((real_values-prediction)/prediction))
#rfMAE <- mean(abs(prediction-real_values))

#result_table <- data.frame(
#  Metric = c("MSE", "RMSE", "MAE", "MAPE"),
#  Value = c(rfMSE, rfRMSE, rfMAE, rfMAPE)
#)
